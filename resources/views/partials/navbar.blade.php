<header class="sticky top-0 z-50 w-full bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-sm border-b border-slate-200 dark:border-slate-800">
    <div class="container mx-auto flex items-center justify-between px-4 py-3">
        <div class="flex items-center gap-8">
            ...
            <nav class="hidden md:flex items-center gap-6">
                @if (Request::is('admin*'))
                    ...
                    <a class="text-sm font-bold text-primary dark:text-primary" href="{{ route('admin.dashboard') }}">Admin Dashboard</a>
                    <a class="text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-primary" href="{{ route('home') }}">View Site →</a>
                @else
                    {{-- Links for the public site --}}
                    ...
                    @auth
                        ...
                        {{-- WISHLIST (FAVORITE) LINK ADDED HERE --}}
                        <a href="{{ route('wishlist.index')}}" class="text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-primary">Wishlist</a>
                        
                        @if (Auth::user()->is_admin)
                            <a class="text-sm font-bold text-primary dark:text-primary hover:underline" href="{{ route('admin.dashboard') }}">Admin Panel</a>
                        @endif
                    @endauth
                @endif
            </nav>
        ...
        <div class="flex items-center gap-4">
            ...
            @else
                <a href="{{ route('bookings.index') }}" class="relative rounded-full p-2 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800">
                    <span class="material-symbols-outlined">bookings</span>
                </a>
                ...
                <div class="relative" x-data="{ open: false }" @click.away="open = false">
                    ...
                    <div x-show="open" style="display: none;" class="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-800 rounded-md shadow-lg py-1 z-50">
                        ...
                        <a href="{{ route('profile.show') }}" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-slate-700">Your Reviews</a>
                        
                        {{-- WISHLIST LINK REMOVED FROM DROPDOWN --}}
